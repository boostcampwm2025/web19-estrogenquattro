    ...
    >
      ...
      >
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            onClick={handleClose}
            aria-label={t(($) => $.leaderboard.closeModal)}
        ...
        <div className="mb-4 text-center">
          <p className="text-sm text-amber-700">
            {t(($) => $.leaderboard.seasonTimer)}
          </p>
          <p className="text-2xl font-bold text-amber-900">
            {seasonTime.days}
            {t(($) => $.leaderboard.day)} : {formatTime(seasonTime.hours)} :{" "}
            ...
            {Object.entries(POINT_TYPES).map(([key, value]) => (
              ...
              >
                {t(
                  ($) =>
                    $.leaderboard.tabs[
                      value as keyof typeof $.leaderboard.tabs
                    ],
                )}
              </button>
            ))}
        ...
        >
          ...
          <h3 className="relative mb-3 flex items-center justify-center gap-3 py-3 text-center text-lg font-bold text-amber-900">
            ...
            <span>
              {t(
                ($) =>
                  $.leaderboard.badges[
                    selectedTab as keyof typeof $.leaderboard.badges
                  ],
              )}
            </span>
            <Sparkles className="h-4 w-4 animate-pulse text-amber-600" />
          ...
          <div className="mb-2 grid grid-cols-4 gap-2 border-b-2 border-amber-900/30 pb-2 text-sm font-bold text-amber-700">
            ...
            <span className="text-center">
              {t(($) => $.leaderboard.headers.profile)}
            </span>
            <span className="text-center">
              {t(($) => $.leaderboard.headers.githubName)}
            </span>
            <span className="text-center">
              {selectedTab === POINT_TYPES.ALL
                ? t(($) => $.leaderboard.headers.point)
                : selectedTab === POINT_TYPES.FOCUSED
                  ? t(($) => $.leaderboard.headers.time)
                  : t(($) => $.leaderboard.headers.count)}
            </span>
          </div>
