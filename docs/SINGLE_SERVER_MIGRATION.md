...
| **2.2** | 인증 시스템 마이그레이션 | `feat: GitHub OAuth 인증 마이그레이션` | [x] |
| **B** | 🧪 OAuth 로그인 테스트 | - | [x] |
| **2.3** | Socket.io 핸들러 마이그레이션 | `feat: Socket.io 핸들러 마이그레이션` | [x] |
| **C** | 🧪 실시간 이동 동기화 테스트 | - | [x] |
| **2.4** | GitHub 폴링 마이그레이션 | `feat: GitHub 폴링 서비스 마이그레이션` | [x] |
| **D** | 🧪 GitHub 커밋 감지 테스트 | - | [x] |
| **3.1** | 정적 파일 서빙 설정 | `feat: 정적 파일 서빙 및 SPA 폴백 설정` | [x] |
| **3.2** | 빌드 스크립트 작성 | `chore: 빌드 스크립트 추가` | [x] |
| **3.3** | PM2 배포 설정 | `chore: PM2 배포 설정` | [x] |
| **E** | 🧪 전체 통합 + 부하 테스트 | - | [x] |
| **완료** | main 브랜치에 머지 | - | [ ] |
