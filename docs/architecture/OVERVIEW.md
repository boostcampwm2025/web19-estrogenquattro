...
│  ├─ GithubModule    : GitHub REST Events 폴링                    │
│  ├─ RoomModule      : 랜덤/선택 방 배정                          │
│  ├─ TaskModule      : Task CRUD                                  │
│  ├─ FocusTimeModule : 집중 시간 관리                             │
│  ├─ PointModule     : 포인트 시스템                              │
│  ├─ UserPetModule   : 펫 시스템                                  │
│  └─ SchedulerModule : 일일 정산/시즌 리셋                        │
├─────────────────────────────────────────────────────────────────┤
│  Infrastructure                                                  │
...
┌─────────────────────────────────────────────────────────────────┐
│                      GitHub API                                  │
│  ├─ /users/{username}/events/public (활동 폴링)                  │
│  └─ /users/{username}, /user/following/* (프로필/팔로우)         │
└─────────────────────────────────────────────────────────────────┘
```
...
│   │   ├── player/         # 플레이어 관리
│   │   ├── point/          # 포인트 시스템
│   │   ├── pointhistory/   # 포인트 히스토리/활동 랭킹
│   │   ├── room/           # 방 관리
│   │   ├── scheduler/      # 정산/시즌 리셋 스케줄러
│   │   ├── task/           # Task CRUD
│   │   └── userpet/        # 펫 시스템
...
```
사용자 접속 → 폴링 시작 (120초 간격) → REST Events API 조회 → 변경 감지 → progress_update 브로드캐스트
```
